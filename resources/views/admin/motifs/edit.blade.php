<x-admin-layout>
    ...
    <div class="max-w-6xl mx-auto">
        <form action="{{ route('admin.motifs.update', $motif->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
                    ...
                    {{-- Upload Area --}}
                    <div class="space-y-4">
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300">Ganti File Grafis <span class="text-xs text-slate-400 font-normal">(Opsional)</span></label>
                    ...
                    <div class="mt-6 flex flex-col items-center">
                        ...
                        <div class="w-[200px] h-[200px] bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm flex items-center justify-center relative p-2">
                            {{-- Preloaded motif image path --}}
                            <img id="image-preview" src="{{ asset($motif->path_file) }}" data-default="{{ asset($motif->path_file) }}" alt="Preview" class="max-w-full max-h-full object-contain">
                        </div>
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 font-semibold mt-2">Gambar yang sedang aktif di database.</span>
                    </div>

                    {{-- Status Checkbox --}}
                    <div class="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
                        <label class="flex items-center cursor-pointer group">
                            <input type="checkbox" name="is_active" value="1" class="w-4 h-4 rounded-md text-emerald-600 focus:ring-emerald-500 focus:border-emerald-500 border-slate-300 dark:bg-slate-900 dark:border-slate-700 shadow-sm transition-all duration-200 @error('is_active') border-red-500 @enderror" {{ old('is_active', $motif->is_active ? '1' : '0') == '1' ? 'checked' : '' }}>
                            <div class="ml-3">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 md:p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="kategori" id="kategori" class="w-full px-4 py-3 text-sm bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:bg-white dark:focus:bg-slate-950 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:text-white transition-all duration-200 capitalize @error('kategori') border-red-500 ring-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ old('kategori', $motif->kategori) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </select>
                        @error('kategori')
                            <p class="text-xs text-red-600 dark:text-red-400 font-semibold mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="5" placeholder="Tuliskan deskripsi detail motif di sini..." class="w-full px-4 py-3 text-sm bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:bg-white dark:focus:bg-slate-950 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:text-white transition-all duration-200 @error('description') border-red-500 ring-red-500 focus:border-red-500 focus:ring-red-500 @enderror">{{ old('description', $motif->description) }}</textarea>
                        @error('description')
                            <p class="text-xs text-red-600 dark:text-red-400 font-semibold mt-1">{{ $message }}</p>
                        @enderror
                    </div>
